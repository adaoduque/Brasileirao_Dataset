...
function getRodadas( $html ) {
	...
	$html.find('tr').each( function () {
		...
		if( $tds.length == 1 && $tds.attr('bgcolor') == '#008000' ) {
			...
			//rodada = $tds.text();
			//isRodada = true;
		}else if( $tds.length == 1 && $tds.attr('bgcolor').toUpperCase() == '#DBEADB' && checkDate( $tds.text() ) ) {
			diaRodada   =  parseDay( $tds.text() );
			dataRodada  =  parseDate( $tds.text() );
			isContinue = true;
		}else if( $tds.length == 1 && checkClube( $tds.text() ) && ( $tds.attr('bgcolor').toUpperCase() == '#E3E2CE' || $tds.attr('bgcolor').toUpperCase() == '#E7E7DC' || $tds.attr('bgcolor').toUpperCase() == '#E1E1D5' || $tds.attr('bgcolor').toUpperCase() == '#DBDBDB' || $tds.attr('bgcolor').toUpperCase() == '#DFDFD2' ) ) {
		...
		}else if( checkLocal( $tds.text() ) && clube1 != '' && clube2 != '' ) {
		...
		}else if( $tds.length == 1 && $tds.attr('colspan') == 5 && $tds.attr('bgcolor').toUpperCase() == '#DBEADB' ) {
			diaRodada   =  parseDay( $tds.text() );
			dataRodada  =  parseDate( $tds.text() );
